-- Re-declares save_pair_profiles (migration 076) to also write the two
-- new columns from migration 077 (caption_image_url/position) — same
-- function signature, so create-or-replace is enough, no drop needed.
  ...
  loop
    ...
    loop
      insert into profile_characters (
        ...
        caption_image_url, caption_image_position
      ) values (
        ...
        char_rec->>'caption_image_url', char_rec->>'caption_image_position'
      )
