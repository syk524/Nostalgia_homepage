-- The app's own save flow (saveProfiles in lib/actions/characters.ts) did
-- a wholesale delete-then-reinsert of a pair's profiles/characters/
-- sections/timeline as several separate PostgREST calls — not atomic, so
-- a failure partway through (e.g. an insert rejected because the
-- schema cache hadn't picked up a just-added column yet) left the old
-- rows already deleted and the new ones never written. That's exactly
-- what happened to two profiles' profile_characters rows on 2026-08-24.
--
-- This wraps the same delete + reinsert sequence in a single plpgsql
-- function instead — one function call is one transaction, so any
-- failed statement inside it rolls back everything, leaving the
-- pre-save data untouched rather than half-deleted.
--
-- security invoker (the default, stated explicitly) — NOT definer: this
-- must keep running as the calling user so the existing editor/admin-only
-- RLS policies on these four tables still gate every insert/delete here
-- exactly as they did when the app called them directly. No role check
-- is duplicated in this function for that reason; RLS already rejects
-- an unauthorized caller with a permission error, same as before.
...
grant execute on function public.save_pair_profiles(uuid, jsonb) to authenticated;
