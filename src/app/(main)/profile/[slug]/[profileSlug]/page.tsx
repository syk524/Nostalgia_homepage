...
export default async function CharacterPairProfilePage({ params }: { params: Promise<{ slug: string; profileSlug: string }> }) {
  ...
  const activeProfile = pair.pair_profiles.find(p => p.profile_slug === profileSlug)
  if (!activeProfile) notFound()
  // The primary profile's canonical URL is the bare pair slug — avoid two
  // URLs serving byte-identical content.
  if (activeProfile.is_primary) redirect(`/profile/${slug}`)
