...
// The Noir theme's decorative backdrop: a field of individual particles
...
// cursor — styled after reactiveparticles.framer.website's repulsion
// effect — behind the "NUSTALGIO" wordmark scattered across it in a
// noise-dissolved style (an SVG feTurbulence/feDisplacementMap filter,
// the same technique the Figma reference itself uses — applied here to
// real DOM text via CSS `filter: url(#...)` instead of pre-baked letter
// paths). Purely decorative — aria-hidden, pointer-events-none
// throughout — and only ever mounted for a non-default theme (see
// draggable-home-scene.tsx).
export function NoirBackground() {
  ...
  useEffect(() => {
    ...
    ro.observe(canvas)
    if (!reduceMotion) {
      // The canvas itself is pointer-events-none (purely decorative, see
      // the wrapper below), so listening on window is what actually
      // catches pointer moves — they'd never reach an element that
      // can't be hit-tested.
