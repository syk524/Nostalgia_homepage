...
type Bucket = 'user-icons' | 'gallery-images' | 'sticker-images' | 'day-counter-photos' | 'trpg-images' | 'memo-images'
...
export async function uploadImage(file: File, ownerId: string, bucket: Bucket) {
  ...
  const uploadFile = bucket === 'gallery-images' || bucket === 'sticker-images' || bucket === 'day-counter-photos' || bucket === 'trpg-images' || bucket === 'memo-images' ? await optimizeForUpload(file) : file
