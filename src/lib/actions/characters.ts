...
type CharacterPairInput = { title: string; pairImageUrl: string | null; backgroundUrl: string | null; backgroundBlur: number; titleFont: string; titleColor: string; characters: [CharacterInput, CharacterInput] }

export async function createCharacterPair(input: CharacterPairInput) {
  const { title, pairImageUrl, backgroundUrl, backgroundBlur, titleFont, titleColor, characters } = input
  ...
  const { data: pair, error } = await supabase
    ...
    .insert({ title: title.trim(), pair_image_url: pairImageUrl, background_url: backgroundUrl, background_blur: backgroundBlur, title_font: titleFont, title_color: titleColor, created_by: user.id })
    .select('id')
    .single()
...
export async function updateCharacterPair(pairId: string, input: CharacterPairInput) {
  ...
  const { data: updated, error } = await supabase
    .from('character_pairs')
    .update({ title: title.trim(), pair_image_url: pairImageUrl, background_url: backgroundUrl, background_blur: backgroundBlur, title_font: titleFont, title_color: titleColor })
    .eq('id', pairId)
    .select('id')
